...
export async function notifyMerchOrder(params: {
  ...
  const itemRows = params.items
    .map(i => `<tr><td style="padding:4px 0;font-size:14px;">${i.colour}</td><td style="padding:4px 0;font-size:14px;">${i.size}</td><td style="padding:4px 0;font-size:14px;text-align:right;">×${i.quantity}</td></tr>`)
    .join('')
  ...
  const html = `
    <div style="font-family:Georgia,serif;max-width:560px;margin:0 auto;">
      ...
      <div style="background:#ffffff;padding:32px;border:1px solid #e8e0d0;">
        ...
        <table style="width:100%;border-collapse:collapse;margin-top:16px;border-top:1px solid #e8e0d0;padding-top:8px;">
          <tr><td colspan="3" style="padding-top:12px;font-size:11px;letter-spacing:0.1em;text-transform:uppercase;color:#8B6F4E;">Items</td></tr>
          ${itemRows}
        </table>

        <table style="width:100%;border-collapse:collapse;margin-top:8px;">
          ${params.total_kes ? `<tr><td style="padding:6px 0;font-size:13px;color:#8B6F4E;width:120px;">Total</td><td style="padding:6px 0;font-size:14px;font-weight:bold;">KES ${params.total_kes.toLocaleString('en-KE')}</td></tr>` : ''}
  ...
  await sendEmail({
    to: [{ email: from, name: '65 Degrees Coffee' }],
    subject: `New merch order${params.orderRef ? ` ${params.orderRef}` : ''} from ${params.name} — ${params.items.length} item${params.items.length === 1 ? '' : 's'}${params.total_kes ? ` — KES ${params.total_kes.toLocaleString('en-KE')}` : ''}`,
    htmlContent: html,
  })
}
